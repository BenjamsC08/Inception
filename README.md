...
The main difference is that a Host Network will use the ip of the host, but in a Docker Network it will have its own. Docker Network will really isolate the container. With Docker Network, you can use different port inside and outside of the docker, (ex: you can use 8080 outside so all devices on your host network will see it on this port, but inside the docker the service provided will used 80). Communication between containers is faster and smoother inside a Docker network because they can reach each other directly by IP. On the host network they all share localhost.
...
- [Php docker override](https://github.com/docker-library/php/issues/241)
